...
void embind_7(float embind7) {
  lua_pushnumber(ofxOfeliaLua::L, embind7);
  lua_setglobal(ofxOfeliaLua::L, "embind_7");
  }

void embind_8(float embind8) {
  lua_pushnumber(ofxOfeliaLua::L, embind8); 
  lua_setglobal(ofxOfeliaLua::L, "embind_8");
  }
...
EMSCRIPTEN_BINDINGS(my_module) {    
  ...
  emscripten::function("embind_9", &embind_9); 
  emscripten::function("embind_10", &embind_10);
  emscripten::function("embind_11", &embind_11);
  }
...
void ofApp::setup()
{
    ...
    const int ticksPerBuffer = 64; // used to compute the audio buffer len: tpb * blocksize (always 64)
#else
