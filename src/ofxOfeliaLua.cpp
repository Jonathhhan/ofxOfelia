...
bool ofxOfeliaLua::init()
{
    ...
    if (L == nullptr)
    {
        bug("ofelia: failed initializing lua");
        return false;
    }
    /* open libs */
    luaL_openlibs(L);
    ...
    int top; switch (getType(s, top))
    {
        ...
        case LUA_TBOOLEAN:
            if (!argc)
                ...
                bug("ofelia: failed to set the boolean variable '%s'", s->s_name);
            break;
        case LUA_TNUMBER:
            if (!argc)
                setVariable(s);
                ...
                bug("ofelia: failed to set the number variable '%s'", s->s_name);
            break;
        case LUA_TSTRING:
            if (!argc)
                setVariable(s);
                ...
                bug("ofelia: failed to set the string variable '%s'", s->s_name);
            break;
        case LUA_TUSERDATA:
            if (!argc)
                setVariable(s);
                ...
                bug("ofelia: failed to set the pointer variable '%s'", s->s_name);
            break;
        case LUA_TTABLE:
            if (!argc)
                setVariable(s);
